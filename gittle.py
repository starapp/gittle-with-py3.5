...
class Gittle(object):
    ...
    def _setup_fetched_refs(self, refs, origin, bare):
        ...
        heads_base = bytes(heads_base, 'utf-8')
        if bare:
        ...
        for key, value in refs.items():
            tmprefs[bytes.decode(key)] = bytes.decode(value)
        for k, v in list(utils.git.clean_refs(tmprefs).items()):
            # TODO_STAR
            self[bytes(k, 'utf-8')] = bytes(v, 'utf-8')
            # self[k] =v
    ...
    def checkout_all(self, commit_sha=None):
        ...
        # commit_sha_str=bytes.decode(commit_sha)
    ...
    def get_commit_files(self, commit_sha, parent_path=None, is_tree=None, paths=None):
        ...
        for entry in list(tree.items()):
            ...
            if entry.mode == self.MODE_DIRECTORY:
                context.update(
                    self.get_commit_files(entry.sha, parent_path=os.path.join(parent_path, entry.path), is_tree=True,
                                          paths=paths)
                )
                continue
    ...
    def add_remote(self, remote_name, remote_url):
        ...
        bstr = "+refs/heads/*:refs/remotes/%s/*" % remote_name

        if type(remote_name) is str:
            remote_name = bytes(remote_name, 'utf-8')
        if type(remote_url) is str:
            remote_url = bytes(remote_url, 'utf-8')
        tp1 = (b'remote', remote_name)
        tp2 = (b'remote', remote_name)
        config.set(tp1, b'url', remote_url)
        config.set(tp2, b'fetch', bytes(bstr, 'utf-8'))
    ...
    def __getitem__(self, key):
        try:
            # TODO_STAR
            keytmp = bytes.decode(key)
            sha = self._parse_reference(keytmp)
        except:
            raise KeyError(key)
        # TODO_STAR
        sha = bytes(sha, 'utf-8')
